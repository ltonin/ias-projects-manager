...
final class PersonCapacityServiceTest extends TestCase
{
    ...
    {
        [$s,$repo,$people,$p]=$this->context();self::assertSame('standard',$s->effectiveCapacity($p,2027,1)->source);self::assertSame('125.00',$s->effectiveCapacity($p,2027,1)->effectiveHours);
        $o=$s->createOverride($p,$this->input(['available_hours'=>'80']));self::assertSame('80.00',$s->effectiveCapacity($p,2027,1)->effectiveHours);
        $changed=$people->update($p->id,$this->personData('150.00'));self::assertSame('80.00',$s->effectiveCapacity($changed,2027,1)->effectiveHours);self::assertSame('150.00',$s->effectiveCapacity($changed,2027,2)->effectiveHours);
        $s->removeOverride($changed,$o);self::assertSame('150.00',$s->effectiveCapacity($changed,2027,1)->effectiveHours);
    }
    ...
    private function personData(string$c='125.00',string$name='Capacity'):array{return['user_id'=>null,'first_name'=>$name,'last_name'=>'Person','institutional_email'=>strtolower($name).'@example.test','affiliation'=>null,'position_type'=>'researcher','is_internal'=>true,'active_from'=>null,'active_to'=>null,'is_active'=>true,'default_monthly_capacity_hours'=>$c,'notes'=>null];}
}
