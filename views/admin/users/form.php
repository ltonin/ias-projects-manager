    ...
    <div class="col-lg-8">
        ...
        <form class="card card-body shadow-sm" method="post" action="<?= View::escape($urls->to($action)) ?>" novalidate>
            ...
            <div class="row g-3">
                ...
                <?php if(!$isEdit):?><div class="col-12"><label class="form-label" for="linked_person_id">Existing Person (optional)</label><select class="form-select<?= isset($errors['linked_person_id'])?' is-invalid':'' ?>" id="linked_person_id" name="linked_person_id"><option value="">Create and link a new Person automatically</option><?php foreach($availablePeople as$person):?><option value="<?= $person->id ?>" <?= (string)($values['linked_person_id']??'')===(string)$person->id?'selected':'' ?>><?= View::escape($person->fullName().($person->institutionalEmail?' — '.$person->institutionalEmail:'')) ?></option><?php endforeach;?></select><?php if(isset($errors['linked_person_id'])):?><div class="invalid-feedback"><?= View::escape($errors['linked_person_id']) ?></div><?php endif;?><div class="form-text">If no Person is selected, name and email are copied once into a new linked registry record. Other Person details are not inferred.</div></div><?php elseif($linkedPerson!==null):?><div class="col-12"><strong>Linked Person:</strong> <a href="<?= View::escape($urls->to('/admin/people/'.$linkedPerson->id.'/edit')) ?>"><?= View::escape($linkedPerson->fullName()) ?></a><div class="form-text">Editing this User does not overwrite the linked Person.</div></div><?php endif;?>
                <div class="col-md-4">
