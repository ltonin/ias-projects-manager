...
final class PdoAnnualEffortRepository implements AnnualEffortRepository
{
    ...
    public function capacityData(array$ids,int$year):array
    {
        ...
        $s=$this->connection()->prepare("SELECT id,default_monthly_capacity_hours FROM people WHERE id IN ($marks)");$s->execute($ids);foreach($s->fetchAll()as$r)$out[(int)$r['id']]=['standard'=>(string)$r['default_monthly_capacity_hours'],'overrides'=>[],'months'=>[]];
        ...
        $s=$this->connection()->prepare("SELECT pp.person_id,a.month,COALESCE(SUM(a.planned_hours),0.00) hours FROM person_hour_allocations a JOIN project_participants pp ON pp.id=a.project_participant_id WHERE a.year=? AND a.planned_hours <=> a.actual_hours AND a.planned_hours IS NOT NULL AND pp.person_id IN ($marks) GROUP BY pp.person_id,a.month");$s->execute([$year,...$ids]);foreach($s->fetchAll()as$r)$out[(int)$r['person_id']]['months'][(int)$r['month']]=['hours'=>(string)$r['hours']];
        $s=$this->connection()->prepare("SELECT pp.person_id,COUNT(*) divergent FROM person_hour_allocations a JOIN project_participants pp ON pp.id=a.project_participant_id WHERE a.year=? AND NOT(a.planned_hours <=> a.actual_hours) AND pp.person_id IN ($marks) GROUP BY pp.person_id");$s->execute([$year,...$ids]);foreach($s->fetchAll()as$r)$out[(int)$r['person_id']]['divergent']=(int)$r['divergent'];
        return$out;
    }
    ...
    public function save(int$p,int$y,array$changes,string$expected,?int$manager):int
    {
        ...
        try{
            ...
            $lock=$pdo->prepare($this->selectSql().' WHERE pp.project_id=:p AND a.year=:y AND a.work_package_id IS NOT NULL ORDER BY wp.code,pp.id,a.month,a.id FOR UPDATE');$lock->execute(['p'=>$p,'y'=>$y]);$rows=array_map(fn(array$r)=>$this->hydrate($r),$lock->fetchAll());
            ...
            $by=[];foreach($rows as$r)$by[$r->workPackageId.'-'.$r->projectParticipantId.'-'.$r->month]=$r;$changed=0;
            foreach($changes as$c){$key=$c['work_package_id'].'-'.$c['participant_id'].'-'.$c['month'];$row=$by[$key]??null;if($row!==null&&$row->plannedHours!==$row->actualHours)throw new \InvalidArgumentException('A divergent allocation must be resolved in its detailed record.');$planned=$c['value'];$actual=$c['value'];
                ...
                if($planned===null&&$actual===null&&$row->notes===null){$s=$pdo->prepare('DELETE FROM person_hour_allocations WHERE id=:id');$s->execute(['id'=>$row->id]);$changed++;continue;}
