...
final class PersonHourAllocationController
{
    ...
    {
        $this->authorization->user();[$project,$participant]=$this->context($p);$listing=$this->service->listing($participant,$this->request->queryData());
    ...
    public function create(array$p):Response
    {
        ...
        $this->flash->add('success',$created->monthLabel().' allocation was created.');return Response::redirect($this->urls->to($this->base($project,$participant).'/'.$created->id));
    }
    ...
    public function update(array$p):Response
    {
        ...
        if($allocation->workPackageId===null){
            ...
            return Response::redirect($this->urls->to($this->base($project,$participant).'/'.$updated->id));
        }
        ...
        $this->flash->add('success',$updated->monthLabel().' allocation was updated.');return Response::redirect($this->urls->to($this->base($project,$participant).'/'.$updated->id));
    }
    public function removeForm(array$p):Response
    {
        [,,$project,$participant]=$this->management($p);$allocation=$this->allocation($p,$project,$participant);
        return new Response($this->view->render('person_hour_allocations/remove',['title'=>'Remove allocation','project'=>$project,'participant'=>$participant,'allocation'=>$allocation->withoutNotes(),'csrfToken'=>$this->csrf->token()]));
    }
    public function remove(array$p):Response
    {
        ...
        return Response::redirect($this->urls->to($this->base($project,$participant)));
    }
    ...
    private function emptyValues():array{return['work_package_id'=>'','year'=>(string)(int)date('Y'),'month'=>(string)(int)date('n'),'planned_hours'=>'','actual_hours'=>'','notes'=>''];}
    private function values(PersonHourAllocation$a):array{return['work_package_id'=>$a->workPackageId===null?'':(string)$a->workPackageId,'year'=>(string)$a->year,'month'=>(string)$a->month,'planned_hours'=>$a->plannedHours??'','actual_hours'=>$a->actualHours??'','notes'=>$a->notes??''];}
