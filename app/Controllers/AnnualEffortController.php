...
final class AnnualEffortController
{
    ...
    {
        $u=$this->authorization->user();$person=$this->currentPerson->get();$project=$this->project($p);$this->requireCsrf();$year=$this->year($this->request->post('year'),$project);$token=(string)$this->request->post('concurrency_token','');$payload=$this->request->post('allocations',[]);
        if(!is_array($payload))return$this->render($this->service->page($project,$year,$u,$person),'Malformed effort grid payload.',422,[]);
    ...
    private function readOnlyPage(\App\Models\AnnualEffortPage$p):\App\Models\AnnualEffortPage{return new \App\Models\AnnualEffortPage($p->project,$p->year,false,$p->sections,$p->projectMonthlyHours,$p->projectAnnualHours,$p->workPackagesWithEffort,$p->participantsWithEffort,$p->capacity,$p->unassigned,'',$p->divergentCount,$p->currentMonth);}
